...
using WidgetNamespace.Items;
using Xunit;

namespace WidgetNamespace.Tests.Definitions;

/// <summary>
/// Golden-file parity for the widget item's code-first definition, pinned through the harness's
...
/// </summary>
public class WidgetItemGoldenTests {
  private static string GoldenPath([CallerFilePath] string here = "") =>
    Path.Combine(Path.GetDirectoryName(here)!, "..", "goldens", "WidgetItem.json");

  [Fact]
  public void Definition_reproduces_its_golden() {
    ExItemDef def = WidgetItems.Definitions("fixture").Single();
    ...
    Assert.True(
      ...
      $"widget item def diverged from its golden:\n{normalized}"
    );
  }
}
