...
**Status** live   **Mod** `exlib` owns the graph substrate and the base pipe blocks - `BlockPipe`/`BlockEntityPipe`, `BlockNetworkNode`, `BlockNetworkModSystem` and `ChimneyVent` live in `src/ExpandedLib/Blocks/Networks` (since 2026-08-07), `PipeNetwork`/`PipeNetworkState` in `src/Networks`; `iiex` owns the plated tier and the network registration; `iiex` owns the cast tier + every fitting; `hpex` owns the rolled tier (segments only).
