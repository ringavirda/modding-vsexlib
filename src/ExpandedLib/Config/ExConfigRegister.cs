...
public sealed class ExConfigRegister<TConfig> : IExConfigAccess
  ...
  /// <inheritdoc/>
  public string ExportJson() => JsonConvert.SerializeObject(Config);

  /// <inheritdoc/>
  public void ImportJson(string json) {
    TConfig? imported;
    try {
      imported = JsonConvert.DeserializeObject<TConfig>(json);
    } catch (Exception e) {
      (_api?.Logger ?? _noopLogger).Warning(
        "[{0}] Could not import synced config '{1}'. {2}",
        _modId,
        _fileName,
        e
      );
    ...
    Sanitize(imported, _api?.Logger ?? _noopLogger);
    Config = imported;
  }
