...
public abstract class BlockEntityMultiblockStructure
  ...
  /// <summary>Declares the fields this structure persists beyond <c>StructureComplete</c> above. Called
  ...
  /// <summary>Test seam: runs one monitor tick exactly as the registered listener would - recomputes
  /// the rotation, then detects a completion/breakage transition and starts or stops the hosted
  /// process the same way <see cref="OnMonitorStructureTick"/> does.</summary>
  internal void DriveMonitorTick() => OnMonitorStructureTick(0f);

  /// <summary>Test seam: recomputes this structure's rotation, the same protected path
  /// <see cref="UpdateStructureRotation"/> uses on every load and monitor tick. When
  /// <paramref name="orientationOrSide"/> is given, it is written to whichever orientation-bearing
  /// variant key ("side" or "orientation") the block's variant map already carries before the
  /// recompute, so a test can rotate and re-derive the angle in one call; when null, the block's
  /// current variant is read as-is, matching a bare <c>UpdateStructureRotation()</c> call.</summary>
