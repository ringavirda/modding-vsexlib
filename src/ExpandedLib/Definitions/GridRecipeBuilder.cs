...
public sealed class GridRecipeBuilder {
  ...
  /// <summary>Sets an arbitrary top-level key on the recipe, for a grid-recipe field without a dedicated
  /// method (e.g. <c>recipeGroup</c>, <c>shapeless</c>).</summary>
...
/// <summary>
/// Fluent builder for one recipe ingredient - a slot in a grid recipe, or a barrel or other recipe
/// ingredient: <c>{ type, code[, name, allowedVariants, quantity, isTool] }</c>. Only set keys are emitted.
/// </summary>
public sealed class IngredientBuilder {
  ...
  /// <summary>Sets the wildcard capture <c>name</c> and its <c>allowedVariants</c>, binding a <c>*</c> in the
  /// code to a named group the output can reuse (e.g. <c>metal</c> over <c>[iron, steel]</c>).</summary>
  ...
  /// <summary>Shorthand for the iron/steel metal capture: <c>Named("metal", "iron", "steel")</c>.</summary>
  ...
  /// <summary>Sets an arbitrary ingredient key, for a field without a dedicated method (e.g. <c>litres</c>
  /// on a barrel ingredient, or <c>toolDurabilityCost</c>).</summary>
