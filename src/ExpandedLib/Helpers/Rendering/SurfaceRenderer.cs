...
/// <summary>Base for renderers that draw a flat, textured horizontal surface (a liquid line) inside a block.</summary>
public abstract class SurfaceRenderer : IRenderer {
  ...
  // One uploaded quad per footprint box, or a single combined mesh when combine is true.
  ...
  /// <param name="rotationY">Y rotation, in radians, matching the block's visual shape.</param>
  /// <param name="combine">True merges all boxes into one always-drawn mesh; false keeps one mesh per box, drawn via <see cref="SelectMeshIndex"/>.</param>
  ...
  /// <summary>Builds one flat quad for a footprint box: a top-down UV projection plus a transform that scales the unit quad onto the box and lays it flat.</summary>
  private static MeshData BuildQuad(Cuboidf box) {
    ...
    // UV mapped from the top-down projection of the box (0-16 px to 0-1 UV).
    quad.Uv =
    [
      box.X2 / 16f,
      box.Z2 / 16f,
    ...
    // Matrixf post-multiplies: the rightmost call applies first (Scale, RotateX, Translate).
  ...
  public void OnRenderFrame(float deltaTime, EnumRenderStage stage) {
    ...
    // A texture bind failure aborts the frame; nothing is drawn untextured.
  ...
  /// <summary>Binds the surface texture; returning false skips the draw for this frame.</summary>
  protected abstract bool BindSurfaceTexture(IRenderAPI render);

  /// <summary>Which mesh to draw from <see cref="MeshRefs"/>; defaults to 0.</summary>
