...
/// <summary>
/// Catches a code-first definition registered after <see cref="ExDefinitionModSystem.AssetsLoaded"/>
/// already injected: the object loader never sees it and the world looks fine, since nothing else in
/// the pipeline says so. Reads <see cref="ExDefinitions"/> directly rather than through
...
/// injection has actually run once in this process: the client side, where
/// <see cref="ExDefinitionModSystem"/> never loads, and a harness path that never replays it.
/// </summary>
    ...
    if (ExDefinitions.InjectionRan) {
      foreach (ExBlockDef def in ExDefinitions.Blocks)
        if (def.Domain == domain && !ExDefinitions.WasInjected(def.Location))
          errors.Add(Message("block", def.Domain, def.Code));
      ...
      foreach (ExRecipeDef def in ExDefinitions.Recipes)
        if (def.Domain == domain && !ExDefinitions.WasInjected(def.Location))
          errors.Add(Message("recipe", def.Domain, def.Code));
    }
    return new CheckResult("LateDefinition", domain, errors);
  }

  private static string Message(string kind, string domain, string code) =>
    $"{domain}:{code} - {kind} definition registered too late to be built; register it from "
