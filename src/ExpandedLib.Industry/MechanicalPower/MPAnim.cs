...
public static class MPAnim {
  ...
  /// <c>[0, totalFrames)</c>; one full revolution (2*pi) advances exactly one cycle. Returns 0 when
  ...
  public static float AdvanceFrame(
    ...
    if (totalFrames <= 1)
      return 0f;
    // AngleRadDistance gives the signed shortest delta, so wrapping past 2*pi or reversing advances the
  ...
  /// <c>0..2*pi</c> maps onto frame <c>0..totalFrames</c> and wraps at <c>2*pi</c>. Use
