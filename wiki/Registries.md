...
several dlls, but only one of them may contain mod systems at all.** exlib itself ships
`exlib.dll` and `exlib.industry.dll` in one folder and hits this limit on its own account -
`exlib.industry.dll` is a **module**, exlib's general mechanism for an assembly that extends a
framework or a mod without a `ModSystem` of its own, driven through a host mod's lifecycle
instead. See [Modules](Modules) for identity, the two shipping forms, the phase table and the
worked example.
