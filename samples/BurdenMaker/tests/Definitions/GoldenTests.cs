...
/// <summary>
/// Golden-file parity for every burdenmaker code-first def: each def must reproduce its committed
/// <c>goldens/burdenmaker/{Location.Path}</c> file, and the golden set must exactly cover the defs.
/// </summary>
public class GoldenTests {
  private const string Domain = "burdenmaker";
  private static readonly Assembly Mod =
    typeof(Blocks.BlockBurdenmaker).Assembly;
  private static readonly string GoldenRoot = System.IO.Path.Combine(
    RepoPaths.Mod("burdenmaker"),
    "tests",
    "goldens"
  );
