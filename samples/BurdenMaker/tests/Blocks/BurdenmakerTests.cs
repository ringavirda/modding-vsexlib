...
public class BurdenmakerTests {
  private static (
    ...
    world.RegisterItem("burdenmaker:burden");
  ...
  public void Every_tank_can_actually_reach_its_configured_capacity() {
    ...
    for (
      int i = 0;
      i < 64 && be.FluxUnits < BurdenMakerValues.BurdenmakerFluxCapacity;
      i++
    )
      be.TryLoadFlux(
    ...
    Assert.Equal(BurdenMakerValues.BurdenmakerFluxCapacity, be.FluxUnits);

    for (
      int i = 0;
      i < 64 && be.OreUnits < BurdenMakerValues.BurdenmakerOreCapacity;
      i++
    )
      be.TryLoadOre(
    ...
    Assert.True(be.BurdenUnits <= BurdenMakerValues.BurdenmakerBunkerCapacity);
  }
  ...
  public void The_gate_refuses_when_nothing_is_loaded() {
    ...
    Assert.Equal("burdenmaker-nothingloaded", error);
  ...
  public void The_gate_refuses_while_the_basin_still_holds_a_batch() {
    ...
    Assert.False(be.ToggleGate(out string? error));
    Assert.Equal("burdenmaker-emptybunker", error);
