...
/// <summary>
/// Golden-file parity for every handmill code-first def: each def must reproduce its committed
/// <c>goldens/handmill/{Location.Path}</c> file, and the golden set must exactly cover the defs.
/// </summary>
public class GoldenTests {
  private const string Domain = "handmill";
  private static readonly Assembly Mod = typeof(Blocks.BlockShaft).Assembly;
  private static readonly string GoldenRoot = System.IO.Path.Combine(
    RepoPaths.Mod("handmill"),
    "tests",
    "goldens"
  );
