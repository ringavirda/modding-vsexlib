...
/// <summary>
/// Golden-file parity for every smokestack code-first def: each def must reproduce its committed
/// <c>goldens/smokestack/{Location.Path}</c> file, and the golden set must exactly cover the defs.
/// </summary>
public class GoldenTests {
  private const string Domain = "smokestack";
  private static readonly Assembly Mod =
    typeof(Blocks.BlockSmokeStackIntake).Assembly;
  private static readonly string GoldenRoot = System.IO.Path.Combine(
    RepoPaths.Mod("smokestack"),
    "tests",
    "goldens"
  );
