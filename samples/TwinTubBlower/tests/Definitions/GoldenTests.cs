...
/// <summary>
/// Golden-file parity for every twintubblower code-first def: each def must reproduce its committed
/// <c>goldens/twintubblower/{Location.Path}</c> file, and the golden set must exactly cover the defs.
/// </summary>
public class GoldenTests {
  private const string Domain = "twintubblower";
  private static readonly Assembly Mod =
    typeof(Blocks.BlockTwinTubMPBlower).Assembly;
  private static readonly string GoldenRoot = System.IO.Path.Combine(
    RepoPaths.Mod("twintubblower"),
    "tests",
    "goldens"
  );
